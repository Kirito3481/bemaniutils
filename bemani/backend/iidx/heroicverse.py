...
class IIDXHeroicVerse(IIDXBase):
    ...
    GAME_CHART_TYPE_B14: Final[int] = 5
    ...
    def handle_IIDX27music_crate_request(self, request: Node) -> Node:
        ...
        for song in all_songs:
            ...
            # B7, N7, H7, A7, L7, B14, N14, H14, A14, L14 order
            for chart in [6, 0, 1, 2, 7, 8, 3, 4, 5, 9]:
                placed = False
    ...
    def handle_IIDX27music_getrank_request(self, request: Node) -> Node:
        ...
        for rivalid in [-1, 0, 1, 2, 3, 4]:
            ...
            if userid is not None:
                ...
                scoredata = self.make_score_struct_new(
                    scores,
    ...
    def handle_IIDX27pc_common_request(self, request: Node) -> Node:
        ...
        # Enable Record play video
        root.add_child(Node.void("play_video"))

        # Show ASIO Logo in Attract Mode
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        lightning_play_data.set_attribute("spnum", str(profile.get_int("lightning_single_plays")))
        lightning_play_data.set_attribute("dpnum", str(profile.get_int("lightning_double_plays")))

        # Lightning settings
        lightning_dict = profile.get_dict("lightning_settings")
        lightning_setting = Node.void("lightning_setting")
        root.add_child(lightning_setting)
        lightning_setting.set_attribute("headphone_vol", str(lightning_dict.get_int("headphone_vol", 10)))
        lightning_setting.add_child(Node.s32_array("slider", lightning_dict.get_int_array("slider", 7, [7, 7, 7, 7, 7, 15, 15])))
        lightning_setting.set_attribute("resistance_sp_left", str(lightning_dict.get_int("resistance_sp_left", 4)))
        lightning_setting.set_attribute("resistance_sp_right", str(lightning_dict.get_int("resistance_sp_right", 4)))
        lightning_setting.set_attribute("resistance_dp_left", str(lightning_dict.get_int("resistance_dp_left", 4)))
        lightning_setting.set_attribute("resistance_dp_right", str(lightning_dict.get_int("resistance_dp_right", 4)))
        lightning_setting.add_child(Node.bool_array("light", lightning_dict.get_bool_array("light", 6, [True, True, True, True, True, True])))
        lightning_setting.add_child(Node.bool("concentration", lightning_dict.get_bool("concentration")))

        # idk what this is for, but sent as 0 always
        spdp_rival = Node.void("spdp_rival")
        root.add_child(spdp_rival)
        spdp_rival.set_attribute("flg", "0")
        ...
        # Enable Premium course (Extra Favorite folders, Quick Retry in Premium Free, bonus VDiscs)
        root.add_child(Node.void("ea_premium_course"))

        # Enable QR code reward feature (https://p.eagate.573.jp/game/2dx/27/event/code/2dcode.html)
        ...
        if game_config.get_bool("force_unlock_songs"):
            ...
            secret.add_child(Node.s64_array("flg4", [-1, -1, -1]))
        else:
        ...
        notes_radar.set_attribute("style", str(self.GAME_CLTYPE_SINGLE))
        notes_radar.add_child(Node.s32_array("radar_score", profile.get_int_array("notes_radar_single", 6)))

        notes_radar = Node.void("notes_radar")
        root.add_child(notes_radar)
        notes_radar.set_attribute("style", str(self.GAME_CLTYPE_DOUBLE))
        notes_radar.add_child(Node.s32_array("radar_score", profile.get_int_array("notes_radar_double", 6)))

        # 遁術 system
        tonjyutsu = Node.void("tonjyutsu")
        root.add_child(tonjyutsu)
        tonjyutsu.set_attribute("platinum_pass", "0")
        tonjyutsu.set_attribute("black_pass", "0")
        ...
        qpro_ticket = Node.void("qpro_ticket")
        root.add_child(qpro_ticket)
        qpro_ticket.set_attribute("ticket_num", str(profile.get_int("qpro_ticket_num")))
        qpro_ticket.set_attribute("total_ticket_num", str(profile.get_int("qpro_total_ticket_num")))
        ...
        event1.set_attribute("last_select_gym_id", str(event1_dict.get_int("last_select_gym_id")))
    ...
    def unformat_profile(self, userid: UserID, request: Node, oldprofile: Profile) -> Profile:
        ...
        if cltype == self.GAME_CLTYPE_SINGLE:
            ...
            if bool(request.child_value("coin_option/is_tdj")):
        ...
        # Lightning settings saving
        lightning_setting = request.child("lightning_setting")
        if lightning_setting is not None:
            lightning_dict = newprofile.get_dict("lightning_settings")
            lightning_dict.replace_int("headphone_vol", int(lightning_setting.attribute("headphone_vol")))
            lightning_dict.replace_int_array("slider", 7, lightning_setting.child_value("slider"))
            lightning_dict.replace_int("resistance_sp_left", int(lightning_setting.attribute("resistance_sp_left")))
            lightning_dict.replace_int("resistance_sp_right", int(lightning_setting.attribute("resistance_sp_right")))
            lightning_dict.replace_int("resistance_dp_left", int(lightning_setting.attribute("resistance_dp_left")))
            lightning_dict.replace_int("resistance_dp_right", int(lightning_setting.attribute("resistance_dp_right")))
            lightning_dict.replace_bool_array("light", 6, lightning_setting.child_value("light"))
            lightning_dict.replace_bool("concentration", bool(lightning_setting.child_value("concentration")))
            newprofile.replace_dict("lightning_settings", lightning_dict)
        ...
        if notes_radar is not None:
            if int(notes_radar.attribute("style")) == self.GAME_CLTYPE_SINGLE:
                newprofile.replace_int_array("notes_radar_single", 6, notes_radar.child_value("radar_score"))
            if int(notes_radar.attribute("style")) == self.GAME_CLTYPE_DOUBLE:
                newprofile.replace_int_array("notes_radar_double", 6, notes_radar.child_value("radar_score"))

        # Tonjyutsu saving
        tonjyutsu = request.child("tonjyutsu")
        if tonjyutsu is not None:
            newprofile.replace_int("platinum_pass", int(tonjyutsu.attribute("platinum_pass")))
            newprofile.replace_int("black_pass", int(tonjyutsu.attribute("black_pass")))
        ...
        # Skin Customize Flag saving
        skin_customize_flg = request.child("skin_customize_flg")
        if skin_customize_flg is not None:
            skin_customize_dict = newprofile.get_dict("skin_customize_flg")
            skin_customize_dict.replace_int("skin_frame_flg", int(skin_customize_flg.attribute("skin_frame_flg")))
            skin_customize_dict.replace_int("skin_bgm_flg", int(skin_customize_flg.attribute("skin_bgm_flg")))
            newprofile.replace_dict("skin_customize_flg", skin_customize_dict)
